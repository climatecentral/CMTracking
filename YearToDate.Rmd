...
# Year-to-Date: January 2022

```{r YTD Hit Type, echo=FALSE, warning=FALSE, message=FALSE}
## Hit Type
library('openair')
YTDdata2021 <- dataReformatted[which(dataReformatted$month %in% c("January") & dataReformatted$year=="2021"),]
YTDdata2022 <- dataReformatted[which(dataReformatted$month %in% c("January") & dataReformatted$year=="2022"),]
...
theme(legend.title = element_blank())+
    geom_text(data=melted.tvgoalsandhits %>% filter(variable=="Airings" & Month=="January"), aes(label = 5685), nudge_x = 0.4, nudge_y = 50, color="black") +
...
labs(y="Articles", x="") +
theme(legend.title = element_blank()) +
    geom_text(data=melted.articlesgoalsandhits %>% filter(variable=="Articles" & Month=="January"), aes(label = 5313), nudge_x = 0.5, nudge_y = 50, color="black") +
    scale_x_discrete(limits = month.name)
  articlesgoalschart
