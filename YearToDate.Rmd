...
## Hit Type
library('openair')
YTDdata2020 <- dataReformatted[which(dataReformatted$month %in% c("January", "February") & dataReformatted$year=="2020"),]
YTDdata2021 <- dataReformatted[which(tracking.data$month %in% c("January", "February") & tracking.data$year=="2021"),]
...
theme(legend.title = element_blank())+
    geom_text(data=melted.tvgoalsandhits %>% filter(variable=="Airings" & Month=="February"), aes(label = value), nudge_x = 0.4, nudge_y = 50, color="black") +
...
labs(y="Articles", x="") +
theme(legend.title = element_blank()) +
    geom_text(data=melted.articlesgoalsandhits %>% filter(variable=="Articles" & Month=="February"), aes(label = value), nudge_x = 0.5, nudge_y = 50, color="black") +
    scale_x_discrete(limits = month.name)
  articlesgoalschart
```
