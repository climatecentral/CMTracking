...
data <- read.csv('March2021TrackingData.csv')
...
## Hit Type
library('openair')
YTDdata2020 <- dataReformatted[which(dataReformatted$month %in% c("January", "February", "March") & dataReformatted$year=="2020"),]
YTDdata2021 <- dataReformatted[which(dataReformatted$month %in% c("January", "February", "March") & dataReformatted$year=="2021"),]
...
theme(legend.title = element_blank())+
    geom_text(data=melted.tvgoalsandhits %>% filter(variable=="Airings" & Month=="March"), aes(label = value), nudge_x = 0.4, nudge_y = 50, color="black") +
...
labs(y="Articles", x="") +
theme(legend.title = element_blank()) +
    geom_text(data=melted.articlesgoalsandhits %>% filter(variable=="Articles" & Month=="March"), aes(label = value), nudge_x = 0.5, nudge_y = 50, color="black") +
    scale_x_discrete(limits = month.name)
  articlesgoalschart
```
