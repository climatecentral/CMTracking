...
Note: Year-to-date data is not yet viewable for 2021.

```{r setup, include=FALSE}
knitr::opts_knit$set(echo=FALSE, warning=FALSE, message=FALSE)
data <- read.csv("January2021TrackingData.csv")
...
## Hit Type
library('openair')
YTDdata2020 <- dataReformatted[which(dataReformatted$month=="January" & dataReformatted$year=="2020"),]
YTDdata2021 <- dataReformatted[which(tracking.data$month=="January" & tracking.data$year=="2021"),]
...
#subset 2021 data
  ...
  #tv goals
  tv.goals <- c(400, 800, 1200, 1600, 2000, 2400, 2800, 3200, 3600, 4000, 4400, 4800)
  month <- month.name
...
theme(legend.title = element_blank())+
    geom_text(data=melted.tvgoalsandhits %>% filter(variable=="Airings" & Month=="December"), aes(label = "368"), nudge_x = 0.2, nudge_y = 200, color="black") +
...
labs(y="Articles", x="") +
theme(legend.title = element_blank()) +
    geom_text(data=melted.articlesgoalsandhits %>% filter(variable=="Articles" & Month=="December"), aes(label = 213), nudge_x = 0.2, nudge_y = 200, color="black") +
    scale_x_discrete(limits = month.name)
  articlesgoalschart
```
...
```{r, Program Hits, eval=FALSE, echo=FALSE, warning=FALSE, message=FALSE}
#subset 2021 data
  data2021 <-dataReformatted[which(dataReformatted$year==2021),]
  #summarize program hits by month
  library("dplyr")
  programsbymonth <- data2021 %>%
    group_by(month) %>%
